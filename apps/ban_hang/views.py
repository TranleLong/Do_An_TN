...
def _save_hoa_don_from_request(request, hoa_don=None):
    ...
    hoa_don.ngay_hach_toan = _parse_date(data.get('ngay_chung_tu') or data.get('ngay_hach_toan'))
    ...
    for index in range(len(hang_ids)):
        ...
        ty_le_ck = _parse_decimal(ty_le_cks[index] if index < len(ty_le_cks) else 0)
